...
async function bloque1(){
  ...
  ok('y dice quién pregunta', !!l && l.body.p_quien === '749608',
  ...
  if(g){
    ...
    ok('y diciendo quién lo hace, que es lo que el servidor comprueba',
       g.body.p_quien === '749608', JSON.stringify(g.body.p_quien));
  }
...
function bloque2(){
  ...
  const ent = crearEntorno({
    ...
    ls: { hes_store: JSON.stringify(STORE),
          hes_empleado: JSON.stringify({ empno:'749608', nombre:'Quien sea',
...
async function bloque4(){
  ...
  const s = arrancar('747851', []);            // Arturo, asesor: sin permiso
