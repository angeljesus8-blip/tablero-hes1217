...
const EQUIPO = ['Arnulfo Gonzalez Arrieta', 'Angel de Jesus Perea Arias'];
...
function arrancar(puesto, catCaptura){
  ...
  const ent = crearEntorno({
    ...
    ls: { hes_store: JSON.stringify(STORE),
          hes_empleado: JSON.stringify({ empno:'749608',
                                         nombre:'Angel de Jesus Perea Arias',
                                         puesto: puesto }) },
...
async function bloque1(){
  ...
  if(g){
    ...
    ok('y diciendo quién lo hace, que es lo que el servidor comprueba',
       g.body.p_quien === '749608', g.body.p_quien);
  }
